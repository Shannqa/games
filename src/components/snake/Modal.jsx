...
import { setPlaying } from "./stages";

function Modal({ score, gameState, setGameState, modal, setModal }) {
  function startGame() {
    setPlaying(setGameState, setModal);
  ...
  if (modal) {
    if (gameState === "loaded") {
      ...
      return (
        <div className={`${styles.modal}`}>
          <p>Game over</p>
          <div>You earned {score.score} points</div>
